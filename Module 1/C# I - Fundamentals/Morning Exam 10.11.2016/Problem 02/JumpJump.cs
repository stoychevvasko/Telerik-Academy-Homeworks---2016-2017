...
You're at a hiphop party in Schmedeswurtherwesterdeich in Germany, you're probably very drunk(hopefully you passed your C# Fundamentals exam at least) and DJ Tomekk is on the scene. Since DJ Tomekk is in the house, your task is to Jump, jump!. Unfortunately, since you've never been at a German hiphop party with DJ Tomekk in the town of Schmedeswurtherwesterdeich, you'll probably need to learn how to jump. Fortunately, you know a bit of programming, so you can follow the instructions from a single string value.
...
class JumpJump
{
    static void Main()
    {
        ...
        while (true)
        {
            ...
            {
                i -= int.Parse(input[i].ToString());
            }
            else if (IsEven(input[i]))
            {
                i += int.Parse(input[i].ToString());
            }
        }
